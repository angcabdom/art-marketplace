      ...
      methods: {
        post: async (req, res, next) => {
          ...
          try {
            ...
            const link = `${req.protocol}://${req.get('host')}/users/confirmation`;
      ...
      methods: {
        post: async (req, res, next) => {
          ...
          try {
            ...
            await User.new(data);
            await existingActivation.destroy();
      ...
      methods: {
        post: async (req, res, next) => {
          ...
          try {
            ...
            existingRestore.user.login.password = User.hashPassword(form.password);
            existingRestore.user.save();
            await existingRestore.destroy();
            res.sendStatus(StatusCodes.CREATED);
          } catch (e) {
            res.status(StatusCodes.INTERNAL_SERVER_ERROR).send(e);
          }
    ...
    me: {
      ...
      methods: {
        get: async (req, res, next) => {
          ...
          try {
            const user = await User.findOne({ 'login.email': existingUser.login.email.trim() });
            if (!user) {
              return next(new RecordNotFound());
            }
